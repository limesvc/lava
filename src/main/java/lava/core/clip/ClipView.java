...
public class ClipView extends View {
    ...
    public static final int TYPE_RATIO = 4;
    ...
    private final Rect ratioRect = new Rect();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        } else if (clipType == ClipType.RATIO) {
            ratioRect.left = Float.valueOf(mHorizontalPadding).intValue();
            ratioRect.right = ratioRect.left + clipWidth;
            int halfHeight = getHeight() / 2;
            int radiusHeight = Float.valueOf(clipWidth / 2f * ratio).intValue();
            ratioRect.top = halfHeight - radiusHeight;
            ratioRect.bottom = halfHeight + radiusHeight;

            canvas.drawRect(ratioRect, paint);
            drawCorners(canvas, ratioRect);
            drawBorder(canvas, ratioRect);
        }
     ...
     *
     * @param type
     * @param ratio ratio 高/宽 比例
     */
    public void setClipType(int type, float ratio) {
        switch (type) {
            case TYPE_ROUND:
            ...
            case TYPE_RATIO:
                ...
                this.setClipType(ClipType.RATIO);
                break;
            default:
                this.setClipType(ClipType.PALACE);
                break;
    ...
    public enum ClipType {
        CIRCLE(1), RECTANGLE(2), PALACE(3), RATIO(4);
    ...
    private void initData() {
        ...
        mCornerLength = dip2px(context, 42);
    }
